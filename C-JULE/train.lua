...
cmd:option('-K_s', 40, 'number of neighbors for computing affinity between samples. (10-100)')
...
cmd:option('-margin_tr', 1, 'margin for weighted triplet loss. (0.2-1.0)')
...
--local myFile = hdf5.open('datasets/'..opt.dataset..'/data4torch_'..tostring(opt.wn)..'.h5', 'r')
...
for i = 1, num_networks do
  ...
  model:add(dofile('models_def/'..opt.dataset..'_'..opt.Num..'.lua')):cuda()
...
function merge_label_final()
  ...
  for i = 1, #network_table do
    features = extFeature(i)
    --local myFile = hdf5.open('results/' .. opt.dataset .. '/'..tostring(opt.wn)..'_feature_pre_'..tostring(opt.Num)..'_'..tostring(opt.learningRate)..'_'..tostring(opt.eta)..'_'..tostring(opt.epoch_pp)..'_'..tostring(opt.K_s)..'_'..tostring(opt.margin_tr)..'_'..tostring(opt.wt)..'_'..tostring(epoch)..'.h5', 'w')
    local myFile = hdf5.open('results/' .. opt.dataset .. '/feature_pre_'..tostring(opt.Num)..'_'..tostring(opt.learningRate)..'_'..tostring(opt.eta)..'_'..tostring(opt.epoch_pp)..'_'..tostring(opt.K_s)..'_'..tostring(opt.margin_tr)..'_'..tostring(opt.wt)..'_'..tostring(epoch)..'.h5', 'w')
...
function updateCNN()
 ...
 --
  --breakpoint('indices:')
  --print (indices)
 -- myFile = hdf5.open('weight/'..opt.dataset..'/new/'..tostring(opt.wn)..'_new_orgin_weight_'..tostring(opt.wt)..'.h5', 'r')
  myFile = hdf5.open('weight/'..opt.dataset..'/new/new_orgin_weight_'..tostring(opt.wt)..'.h5', 'r')
  w_eights  = myFile:read('weight'):all():float()
  w = torch.Tensor((#w_eights)[1]):fill(1)
  for i = 1, #indices do
    for j = 1, (#indices[i])[1] do
      ind = indices[i][j]
      w[(i - 1) * opt.batchSize + j] = w_eights[ind]      
    end 
  end
  --breakpoint('weight:')                                          
  for t,v in ipairs(indices) do
    ...
    for i = 1, #network_table do
      ...
      if x ~= parameters_table[i] then parameters_table[i]:copy(x) end
       ...
       -- we_ight = torch.CudaTensor(we_ight:size()):copy(we_ight)

        --breakpoint('f_loss')
        --print("orgloss:",f)
        if triplets ~= nil then
          we_ight = torch.CudaTensor(triplets_ind[3]:size()):fill(1)
          for wi = 1,(#triplets_ind[3])[1] do
            we_ight[wi] = w_eight[triplets_ind[3][wi]]
          end
          we_ight = torch.repeatTensor(we_ight,160,1):t()
          f = criterion_triplet:forward(triplets,we_ight)
   --       breakpoint('w:')
          df_dtriplets = criterion_triplet:backward(triplets,we_ight)
          df_do = torch.CudaTensor():rand(outputs:size()):zero()
          df_do = cvt2df_do(df_do, df_dtriplets, triplets_ind)
          network_table[i]:backward(inputs, df_do)
--          wei_ght = network_table[i].weight
  --        bia_s = network_table[i].bias
  --        local myFile = hdf5.open('net/' .. opt.dataset .. '/net_para_'..i..'_'..tostring(epoch)..'.h5', 'w')
   --       myFile:write('weight', wei_ght:float())
   --       myFile:write('bias', bia_s:float())
   --       myFile:close()
        end
    ...
    --  f = f * weight
       ...
       --path_w = ('net/'..opt.dataset..'/net_para'..'_'..tostring(epoch)..'.h5')
       --torch.save(module,path_w)
...
function evalPerf()
  ...
  -- local bs = 100
  for i = 1, #network_table do
    --local myFile = hdf5.open('results/' .. opt.dataset .. '/'..tostring(opt.wn)..'_label_pre_'..tostring(opt.Num)..'_'..tostring(opt.learningRate)..'_'..tostring(opt.eta)..'_'..tostring(opt.epoch_pp)..'_'..tostring(opt.K_s)..'_'..tostring(opt.margin_tr)..'_'..tostring(opt.wt)..'_'..tostring(epoch)..'.h5', 'w')
    local myFile = hdf5.open('results/' .. opt.dataset .. '/label_pre_'..tostring(opt.Num)..'_'..tostring(opt.learningRate)..'_'..tostring(opt.eta)..'_'..tostring(opt.epoch_pp)..'_'..tostring(opt.K_s)..'_'..tostring(opt.margin_tr)..'_'..tostring(opt.wt)..'_'..tostring(epoch)..'.h5', 'w')
